...
while 1:
    ...
    while str(soup_turn).find('block') >= 0:
        ...
        if word_len1 > word_len2:
            print("2개의 선택지" + word)
            kkk = cur.execute("SELECT * FROM test WHERE use = 0 AND word LIKE '" + word[0] + "%' OR word LIKE '" + word[-1] + "%' ORDER BY leng DESC LIMIT 1")
            for kk in kkk:
            ...
            if word[-1] == '릇' or word[-1] == '늣':
                kkk = cur.execute("SELECT * FROM test WHERE use=0 AND word LIKE '" + "늣" +"%' ORDER BY leng DESC LIMIT 1") 
                for kk in kkk:
                    driver.find_element_by_xpath('/html/body/div[3]/div[32]/div/input').send_keys(kk[0])
                    use_word = kk[0]
                    cur.execute("update test set use = 1 where word ='"+kk[0]+"'")
